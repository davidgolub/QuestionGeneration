...
def save_lines(array, path, encode=False):
...
def save_tabbed_lines(array, path):
    ...
    with open(path, "w") as fo:
        success = save_lines(concatenated_array, path)
...
def read_tabbed_lines(path, delimiter='\t', append=False):
    ...
    final_content = []
    if append:
        map(lambda s: final_content.extend(s.split(delimiter)), content)
    else:
        final_content = map(lambda s: s.split(delimiter), content)
