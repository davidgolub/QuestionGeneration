...
class Question:
    ...
    def add_answers(self, answer_model_1, answer_model_2, answer_model_3):
        self.answer_text.append(answer_model_1)
        self.answer_text.append(answer_model_2)
        self.answer_text.append(answer_model_3)
        self.eval()
...
def venn_diagram(questions, output_dir):
    ...
    em_model3_ids = [x for x in questions if questions[x].em[2] == 1]
    model_names = questions[list(questions.keys())[0]].model_names
    print('\nVenn diagram')

    correct_model1 = em_model1_ids
    correct_model2 = em_model2_ids
    correct_model3 = em_model3_ids
    ...
    plt.savefig(os.path.join(output_dir, 'venn_diagram.pdf'))
    plt.close()
    return correct_model1, correct_model2, correct_model3, correct_model1_and_model2, correct_model1_and_not_model2, correct_model2_and_not_model1
...
def get_head_ngram_statistics(questions, correct_model1, correct_model2, correct_model1_and_model2, correct_model1_and_not_model2, correct_model2_and_not_model1, output_dir, num_grams=2, top_count=25, correct_model3=None):
    ...
    hnf_correct_model3 = get_head_ngram_frequencies({qid: questions[qid] for qid in correct_model3}, head_ngrams, num_grams)
    ...
    counts_model3 = [hnf_correct_model3[x] for x in top_bigrams]
    ...
    counts_model3_percent = 100 * np.array(counts_model3) / np.array(counts_total)
    ...
    plt.barh([top_count - x for x in ylocs], counts_model3_percent, height=0.33, alpha=0.5, color='#1A202C', label=top_bigrams)
    plt.barh([top_count - x+0.33 for x in ylocs], counts_model2_percent, height=0.33, alpha=0.5, color='#D13F31', label=top_bigrams  )
    plt.barh([top_count - x+0.66 for x in ylocs], counts_model1_percent, height=0.33, alpha=0.5, color='#72B095', label=top_bigrams  )
    ax.set_yticks([top_count - x + 0.33 for x in ylocs])
    ...
    plt.savefig(os.path.join(output_dir, 'ngram_stats_{0}.pdf'.format(num_grams)))
    plt.close()
...
def compare_models(dataset_file, predictions_m1_file, predictions_m2_file, predictions_m3_file, output_dir, name_m1='Model 1', name_m2='Model 2', name_m3='Model 3'):
    dataset = read_json(dataset_file)
    predictions_m1 = read_json(predictions_m1_file)
    predictions_m2 = read_json(predictions_m2_file)
    predictions_m3 = read_json(predictions_m3_file)

    # Read in data
    total = 0
    questions = {}

    for question, question_id, answerss in \
    zip(dataset['q'], dataset['ids'], dataset['answerss']):
        if len(answerss) == 0:
            answerss = ["<NONE>"]
            continue
        question_id = str(question_id)
        current_question = Question(id=str(question_id), question_text=' '.join(question), ground_truth=answerss, model_names=[name_m1, name_m2, name_m3])
        current_question.add_answers(answer_model_1=safe_dict_access(predictions_m1, question_id), answer_model_2=safe_dict_access(predictions_m2, question_id), answer_model_3=safe_dict_access(predictions_m3, question_id))
    ...
    # Venn diagram
    correct_model1, correct_model2, correct_model3, correct_model1_and_model2, correct_model1_and_not_model2, correct_model2_and_not_model1 = venn_diagram(questions, output_dir=output_dir)
    ...
    get_head_ngram_statistics(questions, correct_model1, correct_model2, correct_model1_and_model2, correct_model1_and_not_model2,
                              correct_model2_and_not_model1, output_dir, num_grams=1, top_count=10, correct_model3=correct_model3)

    # Head Bigram statistics
    get_head_ngram_statistics(questions, correct_model1, correct_model2, correct_model1_and_model2, correct_model1_and_not_model2,
                              correct_model2_and_not_model1, output_dir, num_grams=2, top_count=10, correct_model3=correct_model3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-model3', action='store', dest='predictions_m3', required=True, help='Prediction file for model 3')
    ...
    parser.add_argument('-name3', action='store', dest='name_m3', help='Name for model 3')
    ...
    if results.name_m1 is not None and results.name_m2 is not None:
        compare_models(dataset_file=results.dataset, predictions_m1_file=results.predictions_m1, predictions_m2_file=results.predictions_m2, predictions_m3_file=results.predictions_m3, output_dir=results.output_dir, name_m1=results.name_m1, name_m2=results.name_m2, name_m3=results.name_m3)
    else:
