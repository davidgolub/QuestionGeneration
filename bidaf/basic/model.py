...
def get_multi_gpu_models(config):
    print("PRINTING NAME")
    models = []#gpu_idx - config.gpu_idx
    for gpu_idx in range(config.gpu_idx, config.num_gpus + config.gpu_idx):
        with tf.name_scope("model_{}".format(0)) as scope, tf.device("/{}:{}".format(config.device_type, gpu_idx)):
            model = Model(config, scope, rep=gpu_idx == 0)
            #tf.get_variable_scope().reuse_variables() # Since we are only using a single model
            models.append(model)
    return models
...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        # Margin loss algorithm
        self.top_k_y1_par_idx = tf.placeholder('int32', [N, config.k], name='top_k_y1_par_idx')
        self.top_k_y1_sent_idx = tf.placeholder('int32', [N, config.k], name='top_k_y1_sent_idx')

        self.top_k_y2_par_idx = tf.placeholder('int32', [N, config.k], name='top_k_y2_par_idx')
        self.top_k_y2_sent_idx = tf.placeholder('int32', [N, config.k], name='top_k_y2_sent_idx')

        self.top_k_y1_labels = tf.placeholder('int32', [N], name='top_k_y1_labels')
        self.top_k_y2_labels = tf.placeholder('int32', [N], name='top_k_y1_labels')

        self.loss_mask = tf.placeholder('bool', [N], name='top_k_y1_labels')
    ...
    def _build_forward(self):
        ...
        with tf.variable_scope("main"):
            ...
            self.shaped_logits = logits 
            self.shaped_logits2 = logits2
            self.yp = yp
            self.yp2 = yp2
    ...
    def _build_var_ema(self):
        ...
        ema = self.var_ema
        trainable_variables = [v for v in tf.trainable_variables() if self.scope in v.name]
    ...
    def get_margin_feed_dict(self, top_k_batch, is_train, margin_val=10., supervised=True):
        #print(top_k_batch)
        #print(top_k_batch['predicted_spans'][0:5])
        #print(top_k_batch['predicted_f1_scores'][0:5])
        #print(top_k_batch['best_spans'][0:5])
        #print(top_k_batch['best_f1_scores'][0:5])

        print(np.mean(top_k_batch['best_f1_scores']))
        print(np.mean(top_k_batch['predicted_f1_scores']))

        top_k_y1_par_idx = np.zeros((self.config.batch_size, self.config.k))
        top_k_y2_par_idx = np.zeros((self.config.batch_size, self.config.k))
        top_k_y1_sent_idx = np.zeros((self.config.batch_size, self.config.k))
        top_k_y2_sent_idx = np.zeros((self.config.batch_size, self.config.k))
        top_k_y1_labels = np.zeros((self.config.batch_size))
        top_k_y2_labels = np.zeros((self.config.batch_size))
        loss_mask = top_k_batch['loss_mask']

        num_diff = 0
        for i in range(0, len(top_k_batch['predicted_spans'])):
            cur_top_k_spans = top_k_batch['top_k_spans'][i]
            cur_index = top_k_batch['best_f1_indices'][i]
            cur_f1_scores = top_k_batch['best_f1_scores'][i]
            top_k_f1_scores = top_k_batch['top_k_f1_scores'][i]
            top_k_y1_labels[i] = cur_index
            top_k_y2_labels[i] = cur_index
            for j in range(0, len(cur_top_k_spans)):
                cur_span = cur_top_k_spans[j]
                top_k_y1_par_idx[i][j] = cur_span[0][0]
                top_k_y1_sent_idx[i][j] = cur_span[0][1]
                top_k_y2_par_idx[i][j] = cur_span[1][0]
                top_k_y2_sent_idx[i][j] = cur_span[1][1] - 1

        feed_dict = {}
        feed_dict[self.top_k_y1_par_idx] = top_k_y1_par_idx 
        feed_dict[self.top_k_y1_sent_idx] = top_k_y1_sent_idx 
        feed_dict[self.top_k_y2_par_idx] = top_k_y2_par_idx 
        feed_dict[self.top_k_y2_sent_idx] = top_k_y2_sent_idx
        feed_dict[self.top_k_y1_labels] = top_k_y1_labels
        feed_dict[self.top_k_y2_labels] = top_k_y2_labels
        feed_dict[self.loss_mask] = loss_mask
        return feed_dict 
