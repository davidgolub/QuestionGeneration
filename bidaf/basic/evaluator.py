...
class MultiGPUF1Evaluator(F1Evaluator):
    ...
    def _get_feed_dict(self, batches):
        feed_dict = {}
        dummy_batch = {}
        dummy_batch['best_f1_scores'] = []
        dummy_batch['predicted_f1_scores'] = []
        dummy_batch['loss_mask'] = [False] * self.models[0].config.batch_size
        dummy_batch['predicted_spans'] = []

        for model, (_, data_set) in zip(self.models, batches):
            feed_dict.update(model.get_feed_dict(data_set, False))
            feed_dict.update(model.get_margin_feed_dict(dummy_batch, False))
        return feed_dict
